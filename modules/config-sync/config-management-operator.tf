resource "kubectl_manifest" "config_management_operator_crd" {
  yaml_body = <<YAML
...
resource "kubectl_manifest" "config_management_operator_system_namespace" {
  yaml_body = <<YAML
apiVersion: v1
kind: Namespace
metadata:
...
resource "kubectl_manifest" "config_management_operator_monitoring_namespace" {
  yaml_body = <<YAML
apiVersion: v1
kind: Namespace
metadata:
...
resource "kubectl_manifest" "config_management_operator_clusterrole" {
  yaml_body = <<YAML
...
resource "kubectl_manifest" "config_management_operator_clusterrolebinding" {
  depends_on = [
    kubectl_manifest.config_management_operator_clusterrole
  ]
  yaml_body = <<YAML
apiVersion: rbac.authorization.k8s.io/v1
...
resource "kubectl_manifest" "config_management_operator_sa" {
  depends_on = [
    kubectl_manifest.config_management_operator_system_namespace
  ]
  yaml_body = <<YAML
apiVersion: v1
kind: ServiceAccount
...
resource "kubectl_manifest" "config_management_operator_deployment" {
  depends_on = [
    kubectl_manifest.config_management_operator_sa
  ]
  yaml_body = <<YAML
apiVersion: apps/v1
kind: Deployment
  ...
  template:
    ...
    spec:
      ...
      - command:
        ...
        - --private-registry=
        name: manager
        image: gcr.io/config-management-release/config-management-operator:20230510165643-op
        resources:
          requests:
            cpu: 100m
            memory: 200Mi
